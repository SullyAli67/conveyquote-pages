...
function App() {
  ...
  const addFeeItem = (isDisbursement: boolean) => {
    setFeeConfigItems((prev) => [
      ...prev,
      { label: isDisbursement ? "Additional disbursement" : "Additional legal fee", amount: 0, includes_vat: !isDisbursement, is_disbursement: isDisbursement },
    ]);
  };
    e.preventDefault();
                              ...
                              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center" }}>
                                ...
                                {eta && (
                                  <span style={{ fontSize: "11px", color: overdue ? "#dc2626" : "var(--muted)", fontWeight: overdue ? 700 : 400 }}>
                                    {overdue ? "⚠ Overdue" : "Est. completion"}: {eta.toLocaleDateString("en-GB")}
                                  </span>
                                )}
                                {c.invoice_ref && (
                ...
                {!isLoadingPipeline && pipeline.length > 0 && (
                  <SummaryCard title="Pipeline Summary">
                    <SummaryGrid rows={[
                      ...
                      { label: "Overdue completion", value: String(pipeline.filter((c) => c.eta_date && new Date() > new Date(c.eta_date) && c.case_status !== "completed").length) },
                        ...
                        {referrerPortalData.enquiries.map((enq) => (
                          <div key={enq.reference} className="detail-row">
                            ...
                            <div className="detail-row__value">
                              ...
                              {enq.eta_date && <div style={{ fontSize: "12px", color: "var(--muted)" }}>Est. completion: {new Date(enq.eta_date).toLocaleDateString("en-GB")}</div>}
