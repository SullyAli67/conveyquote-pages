...
function App() {
  ...
  const [referrerEditor, setReferrerEditor] = useState({
    id: null as number | null,
    referrer_name: "", contact_email: "", contact_phone: "",
    referral_fee: "", portal_email: "", portal_active: true, notes: "", password: "",
  });
  const [referrerSaveMessage, setReferrerSaveMessage] = useState("");
                      ...
                      <SummaryCard title="New Client Quote">
                        <form onSubmit={(e) => void handleCreateFirmQuote(e)}>
                          <div className="form-grid">
                            ...
                            <div className="field">
                              <label htmlFor="fqPostcode">Postcode</label>
                            ...
                            <div className="field">
                              ...
                              <input id="fqSignature" type="text" value={firmQuoteForm.email_signature}
                                ...
                                placeholder={firmSession.firm_name} />
                            </div>
                          </div>

                          {/* Fee line items pulled from fee config */}
                          <div style={{ marginTop: "20px" }}>
                            <h4 style={{ marginBottom: "10px", color: "#0f2747" }}>Quote Line Items</h4>
                            <p className="form-note" style={{ marginBottom: "10px" }}>
                              Pre-filled from your saved fee settings. Adjust amounts as needed before saving.
                            </p>
                            <div className="detail-table">
                              {feeConfigItems.map((item, idx) => (
                                <div key={idx} className="detail-row">
                                  ...
                                  <div className="detail-row__value">
                                    <input type="number" step="0.01" value={item.amount} style={{ width: "100px" }}
                            ...
                            {(() => {
                              ...
                              return (
                                <div style={{ marginTop: "12px", padding: "12px 16px", background: "#f7f9fc", borderRadius: "8px" }}>
                                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                                    <span>Legal fees ex VAT</span><strong>£{qd.legalFeesExVat.toFixed(2)}</strong>
                                  </div>
                                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                                    <span>VAT (20%)</span><strong>£{qd.vat.toFixed(2)}</strong>
                                  </div>
                                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                                    <span>Disbursements</span><strong>£{qd.disbursementTotal.toFixed(2)}</strong>
                                  </div>
                                  <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #e5e7eb", paddingTop: "8px", marginTop: "4px" }}>
                                    <strong>Total</strong><strong>£{qd.grandTotal.toFixed(2)}</strong>
                                  </div>
                                </div>
                              );
                            })()}
                          ...
                          {firmQuoteForm.internal_reference && (
                            <div style={{ marginTop: "14px", padding: "12px 16px", background: "#d1fae5", borderRadius: "8px" }}>
                              <p style={{ margin: 0, color: "#065f46" }}>
                                <strong>Quote saved:</strong> {firmQuoteForm.internal_reference}
                              </p>
...
function ReferrerEnquiryForm({ referrerToken, onSuccess }: { referrerToken: string; onSuccess: () => void }) {
  const [form, setForm] = useState({
    ...
    mortgage: "mortgage", firstTimeBuyer: "no", additionalProperty: "no",
    ukResidentForSdlt: "yes", newBuild: "no", sharedOwnership: "no",
    helpToBuy: "no", isCompany: "no", buyToLet: "no",
    giftedDeposit: "no", lifetimeIsa: "no", ownershipType: "individual",
    saleMortgage: "no", managementCompany: "no", tenanted: "no", numberOfSellers: "1",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await res.json();
      if (result.success) {
        setMessage(`Enquiry submitted successfully. Reference: ${result.reference}`);
  ...
  return (
    <form onSubmit={(e) => void handleSubmit(e)}>
      <div className="form-grid">
        <div className="field field--full"><h4 style={{ margin: "0 0 4px", color: "var(--navy)" }}>Client Details</h4></div>
        <div className="field">
        ...
        {isPurchase && <>
          <div className="field">
            ...
            <select name="mortgage" value={form.mortgage} onChange={handleChange}>
              <option value="mortgage">Mortgage</option>
              <option value="cash">Cash</option>
            </select>
          </div>
      ...
      {message && (
        <p className="form-note" style={{ marginTop: "14px", color: message.includes("successfully") ? "#065f46" : "#dc2626", background: message.includes("successfully") ? "#d1fae5" : "#fee2e2", padding: "10px 14px", borderRadius: "8px" }}>
          {message}
        </p>
      )}
