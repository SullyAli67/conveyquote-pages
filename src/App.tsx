...
function App() {
  ...
  // Invoices state
  type InvoiceRow = { reference: string; invoice_ref: string; invoice_json: string; invoice_status: string; client_name: string; assigned_firm_name: string; case_status: string; created_at: string };
                                ...
                                {invoiceDetail && (
                                  <div style={{ display: "flex", gap: "6px", marginTop: "6px", flexWrap: "wrap" }}>
                                    <button type="button" className="muted-button" style={{ minHeight: 30, padding: "0 10px", fontSize: "12px" }}
                                      onClick={() => { setSelectedInvoice(invoiceDetail); setVoidInvoiceMessage(""); }}>
                                      View
                                    </button>
                                    <button
                                      type="button"
...
function ReferrerEnquiryForm({ referrerToken, onSuccess }: { referrerToken: string; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await res.json() as { success: boolean; reference?: string; client_emailed?: boolean; error?: string };
      if (result.success) {
        setSubmitted(true);
        const clientNote = result.client_emailed ? " Quote also sent to client." : "";
        setMessage(`Enquiry submitted. Reference: ${result.reference}.${clientNote}`);
      ...
      <div className="form-footer action-row" style={{ marginTop: "20px" }}>
        <div style={{ width: "100%", marginBottom: "12px" }}>
          <label style={{ display: "flex", alignItems: "flex-start", gap: "10px", cursor: "pointer", fontSize: "14px" }}>
            ...
            />
            <span>
              <strong>Send quote to client now</strong>
              <span style={{ color: "var(--muted)", fontSize: "13px", display: "block", marginTop: "2px" }}>
                Emails the calculated quote directly to the client with an Accept button. Leave unticked to submit for admin review first.
              </span>
            </span>
          </label>
        </div>
        <button type="submit" className="primary-button" disabled={submitting}>
          {submitting ? "Submitting…" : sendToClient ? "Submit & Send Quote to Client" : "Submit Client Enquiry"}
        </button>
      </div>
    </form>
  );
