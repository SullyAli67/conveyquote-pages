...
function App() {
  ...
  const isFirmLoginPage = currentPath === "/firm-login";
  const isFirmPortalPage = currentPath === "/firm-portal";
  ...
  const handleSaveLender = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/save-panel-lender", {
        method: "POST",
  ...
  const handleAssignPanelFirm = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/assign-panel-firm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleUpdateEnquiryStatus = async (newStatus: string) => {
    ...
    try {
      const response = await fetch("/api/update-enquiry-status", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          reference: loadedEnquiry.reference,
