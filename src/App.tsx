...
function App() {
  ...
  const [firmPortalTab, setFirmPortalTabRaw] = useState<FirmPortalTab>(() => {
    // Validate before adopting — a stale "quotes" value left over from the
    // retired My Quotes rail would otherwise hide the portal until the
    // user manually clicked another tab.
  ...
  const handleAdminLogout = async () => {
    ...
    localStorage.removeItem("cq_admin_tab");
  };
  ...
  const handleFirmLogin = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setFirmPassword("");
        // Persist session so page refresh does not log the firm out
        localStorage.setItem("cq_firm_token", result.token);
        localStorage.setItem("cq_firm_session", JSON.stringify({ firm_id: result.firm_id, firm_name: result.firm_name }));
  ...
  const handleFirmLogout = async () => {
    ...
    setFirmEmail("");
    localStorage.removeItem("cq_firm_token");
    localStorage.removeItem("cq_firm_session");
    localStorage.removeItem("cq_firm_tab");
  ...
  // Set ETA on a referred matter — date is chosen explicitly by the firm in the UI
  const handleSetEta = async (reference: string, etaDate: string) => {
    if (!etaDate) return;
    try {
      const res = await fetch("/api/firm-update-case-status", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${firmToken}` },
        body: JSON.stringify({ reference, status: "on_hold", notes: "ETA updated by firm", eta_date_override: etaDate }),
      });
      const result = await res.json() as { success: boolean; eta_date?: string; error?: string };
      if (result.success) {
        setCaseStatusMessage(`Estimated completion date set to ${result.eta_date ? new Date(result.eta_date).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : etaDate}.`);
  ...
  const rebuildApprovedQuoteFromEnquiry = (enquiry: LoadedEnquiry) => {
    ...
    const built = buildQuoteData({
      ...
      rightToBuy: enquiry.right_to_buy || "",
  ...
  useEffect(() => {
    try {
      ...
      // Referrer session
      const savedReferrerToken = localStorage.getItem("cq_referrer_token");
      const savedReferrerSession = localStorage.getItem("cq_referrer_session");
      if (savedReferrerToken && savedReferrerSession) {
        const parsed = JSON.parse(savedReferrerSession) as { referrer_id: number; referrer_name: string };
        setReferrerToken(savedReferrerToken);
        setReferrerSession(parsed);
        // Silently reload portal data in the background
        fetch("/api/referrer-portal-data", { headers: { Authorization: `Bearer ${savedReferrerToken}` } })
          .then((r) => r.json())
          .then((d: unknown) => {
            const data = d as { success: boolean; referrer?: { id: number; referrer_name: string; contact_email: string; referral_fee: number }; enquiries?: Record<string, unknown>[] };
            if (data.success) setReferrerPortalData(data as { referrer: { id: number; referrer_name: string; contact_email: string; referral_fee: number }; enquiries: Record<string, unknown>[] });
          })
          .catch(() => {
            // Token expired — clear and force re-login
            localStorage.removeItem("cq_referrer_token");
            localStorage.removeItem("cq_referrer_session");
            setReferrerToken("");
            setReferrerSession(null);
          });
      }

      // Firm session
      const savedFirmToken = localStorage.getItem("cq_firm_token");
      const savedFirmSession = localStorage.getItem("cq_firm_session");
      if (savedFirmToken && savedFirmSession) {
        const parsed = JSON.parse(savedFirmSession) as { firm_id: number; firm_name: string };
        setFirmToken(savedFirmToken);
        setFirmSession(parsed);
      }
    } catch {
  ...
  const matterSpecificRows: SummaryRow[] = useMemo(() => {
    ...
    if (loadedEnquiry.transaction_type === "purchase") {
      ...
      return [
        ...
        {
          label: "Right to Buy",
          value: prettifyValue(loadedEnquiry.right_to_buy),
        },
      ];
    }
        ...
        {(isFirmPortalPage || (isFirmLoginPage && firmSession)) && firmSession && (
          <section className="card card--form" style={{ marginTop: "24px" }}>
            ...
            {(() => {
              ...
              const baseTabs: FirmPortalTab[] = ["referrals"];
                      ...
                      {(firmPortalData.enquiries as Record<string, unknown>[]).length === 0 ? (
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                          {(firmPortalData.enquiries as Record<string, unknown>[]).map((enq) => {
                            ...
                            // Quote data stored in enquiries.quote_json (parsed by API)
                            const storedQuote = enq.quote as {
                            ...
                            return (
                              <FirmReferredMatterCard
                                ...
                                onOpenAcceptance={() => openAcceptanceModal(ref)}
                                onDecline={(notes) => void handleFirmRespond(ref, "declined", notes)}
                                onUpdateCaseStatus={(status, fallReason, completionDate) => void handleUpdateCaseStatus(ref, status, "", fallReason, completionDate)}
                                onSetEta={(date) => void handleSetEta(ref, date)}
                                onPortalRefresh={() => void loadFirmPortalData(firmToken)}
                              />
                            );
                          })}
                        </div>
                    ...
                    <form onSubmit={(e) => void handleSaveReferrer(e)}>
                      <div className="form-grid">
                        ...
                        <div className="field">
                          ...
                          <input type="number" step="0.01" value={referrerEditor.marketing_fee}
                            onChange={(e) => setReferrerEditor((p) => ({ ...p, marketing_fee: e.target.value }))}
                            placeholder="50.00" />
                        </div>
                        <div className="field">
                          <label>Quote fee markup (£) <span style={{ fontSize: "11px", color: "var(--muted)" }}>Added to client-facing legal fees</span></label>
                          <input type="number" step="0.01" min="0" value={referrerEditor.fee_markup ?? ""}
                            onChange={(e) => setReferrerEditor((p) => ({ ...p, fee_markup: e.target.value }))}
            ...
            <form className="quote-form" onSubmit={(e) => {
              ...
              fetch("/api/referrer-login", {
                ...
                if (result.success) {
                  ...
                  setReferrerPassword?.("");
                  // Persist session so page refresh does not log the referrer out
                  localStorage.setItem("cq_referrer_token", result.token);
                  localStorage.setItem("cq_referrer_session", JSON.stringify({ referrer_id: result.referrer_id, referrer_name: result.referrer_name }));
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
              ...
              <button type="button" className="muted-button" style={{ minHeight: 38, padding: "0 16px" }}
                onClick={() => {
                  ...
                  localStorage.removeItem("cq_referrer_token");
                  localStorage.removeItem("cq_referrer_session");
                  localStorage.removeItem("cq_referrer_tab");
            ...
            {referrerPortalTab === "my_referrals" && (() => {
              ...
              const handleArchive = (ref: string) => {
                ...
                fetch("/api/archive-enquiry", {
                  ...
                  if (r.success) { refreshPortal(); }
                  else { setArchiveMsg((p) => ({ ...p, [ref]: r.error || "Failed." })); }
                }).catch(() => setArchiveMsg((p) => ({ ...p, [ref]: "Something went wrong." })));
              };
              ...
              return (
                <div>
                  ...
                  {referrerPortalData && visibleEnquiries.length === 0 && (
                            ...
                            <div style={{ padding: "18px 20px" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px", flexWrap: "wrap", marginBottom: "10px" }}>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <div style={{ marginTop: "8px" }}>
                                    <button type="button"
                                      style={{ fontSize: "12px", padding: "3px 10px", borderRadius: "6px", border: "1px solid var(--navy)", background: caseOpen ? "var(--navy)" : "transparent", color: caseOpen ? "#fff" : "var(--navy)", cursor: "pointer", fontWeight: 600 }}
                                      onClick={() => setOpenCase(caseOpen ? null : ref)}>
                                      {caseOpen ? "\u25be Close Case" : "\u25b8 View Case"}
                                    </button>
                                  </div>
                                </div>
                                    ...
                                    return (
                                      <div key={m} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
                                        <div style={{ width: "100%", display: "flex", alignItems: "center" }}>
                                          ...
                                          <div style={{ width: "14px", height: "14px", borderRadius: "50%", background: done ? "var(--teal)" : "#e5e7eb", border: `2px solid ${done ? "var(--teal)" : "#d1d5db"}`, flexShrink: 0 }} />
                              ...
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px", marginTop: "6px" }}>
                                ...
                                <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
                                  {requestMsg[ref] && (
                                    <span style={{ fontSize: "12px", color: requestMsg[ref].startsWith("\u2713") ? "#065f46" : "#dc2626" }}>{requestMsg[ref]}</span>
                                  )}
                                  {isActive && (
                                    <button type="button" className="muted-button" style={{ minHeight: 30, padding: "0 10px", fontSize: "12px" }}
                                  ...
                                  {isDraft && (
                                    <button type="button"
                                      ...
                                      onClick={() => handleArchive(ref)}>Archive</button>
                                  )}
                                  {archiveMsg[ref] && (
                                    <span style={{ fontSize: "12px", color: "#dc2626" }}>{archiveMsg[ref]}</span>
                                  )}
                                </div>
                              </div>
                            </div>
                            ...
                            {caseOpen && (
                              <div style={{ borderTop: "1px solid #e5e7eb", background: "#f8fafc" }}>
                                ...
                                <div style={{ padding: "14px 20px", borderBottom: "1px solid #e5e7eb" }}>
                                  <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: "#6b7280", marginBottom: "8px" }}>Matter</div>
                                  {enq.property_address && <div style={{ fontSize: "18px", fontWeight: 700, color: "var(--navy)", marginBottom: "4px" }}>{String(enq.property_address)}</div>}
                                  <div style={{ fontSize: "12px", color: "#6b7280", marginBottom: "10px" }}>Ref: <span style={{ fontFamily: "monospace", color: "#374151" }}>{ref}</span></div>
                                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: "8px 20px" }}>
                                    {[
                                      { label: "Transaction", value: String(enq.transaction_type || "\u2014").replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()) },
                                      { label: "Tenure", value: enq.tenure ? String(enq.tenure).charAt(0).toUpperCase() + String(enq.tenure).slice(1) : "\u2014" },
                                      { label: "Price", value: enq.price ? `\u00a3${Number(enq.price).toLocaleString("en-GB")}` : "\u2014" },
                                      ...(enq.mortgage ? [{ label: "Mortgage", value: enq.mortgage === "mortgage" ? "Yes" : "No (cash)" }] : []),
                                      ...(enq.lender ? [{ label: "Lender", value: String(enq.lender) }] : []),
                                    ].map(({ label, value }) => (
                                      <div key={label}>
                                ...
                                {/* Client */}
                                <div style={{ padding: "14px 20px", borderBottom: "1px solid #e5e7eb" }}>
                                  <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: "#6b7280", marginBottom: "8px" }}>Client</div>
                                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: "8px 20px" }}>
                                    {[
                                      { label: "Name", value: String(enq.client_name || "\u2014"), bold: true },
                                      { label: "Email", value: String(enq.client_email || "\u2014"), bold: false },
                                      { label: "Phone", value: String(enq.client_phone || "\u2014"), bold: false },
                                    ].map(({ label, value, bold }) => (
                                      <div key={label}>
                                        <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", marginBottom: "2px" }}>{label}</div>
                                        <div style={{ fontSize: "13px", color: "#111827", fontWeight: bold ? 600 : 400, wordBreak: "break-word" }}>{value}</div>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                                ...
                                <div style={{ padding: "14px 20px", borderBottom: (Number(enq.referral_fee_payable) === 1 || enq.referrer_note) ? "1px solid #e5e7eb" : undefined }}>
                                  ...
                                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: "8px 20px" }}>
                                    ...
                                    {enq.assigned_firm_name && (
                                      <div>
                                        <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", marginBottom: "2px" }}>Assigned firm</div>
                                        <div style={{ fontSize: "13px", color: "#111827", fontWeight: 600 }}>{String(enq.assigned_firm_name)}</div>
                                      </div>
                                    )}
                                    {(enq.eta_date || enq.target_completion_date) && (
                                      <div>
                                        <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", marginBottom: "2px" }}>{enq.target_completion_date ? "Completion" : "Est. completion"}</div>
                                        <div style={{ fontSize: "13px", color: "#5b21b6", fontWeight: 600 }}>{new Date(String(enq.target_completion_date || enq.eta_date)).toLocaleDateString("en-GB")}</div>
                                      </div>
                                    )}
                                    <div>
                                ...
                                {Number(enq.referral_fee_payable) === 1 && (
                                  <div style={{ padding: "14px 20px", borderBottom: requoteOpen === ref ? "1px solid #e5e7eb" : undefined }}>
                                    ...
                                    <div style={{ display: "flex", gap: "24px", flexWrap: "wrap", alignItems: "flex-end" }}>
                                      <div>
                                        <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", marginBottom: "2px" }}>Referral fee</div>
                                        <div style={{ fontSize: "16px", fontWeight: 700, color: "#7c3aed" }}>\u00a3{Number(enq.referral_fee_amount || 0).toFixed(2)}</div>
                                      </div>
                                      <div>
          ...
          <div className="site-footer__grid">
            ...
            <div>
              ...
              <div className="site-footer__links">
                ...
                <a href="/why-are-solicitors-slow-in-conveyancing">Why Solicitors Are Slow</a>
...
//   • Property address + full matter details
//   • Read-only quote breakdown (from enquiries.quote_json)
//   • Inline quote editor — firm can add/remove/edit line items and save
//   • Status controls (accept/decline/update)
//
...
function FirmReferredMatterCard({
  enq,
  firmToken,
  isAccepted, isDeclined, isCompleted,
  caseStatus, etaDate,
  storedQuote,
  fmt,
  CASE_STATUSES,
  ...
  onUpdateCaseStatus: (status: string, fallReason?: string, completionDate?: string) => void;
  onSetEta: (date: string) => Promise<void> | void;
  onPortalRefresh: () => void;
}) {
  const ref = String(enq.reference || "");
  const [quoteOpen, setQuoteOpen] = useState(false);
  const [editMode, setEditMode] = useState(false);
  const [editLegal, setEditLegal] = useState<QuoteLineItem[]>([]);
  const [editDisb, setEditDisb] = useState<QuoteLineItem[]>([]);
  ...
  // Inline status confirmation state — for statuses that need extra data
  const [pendingStatus, setPendingStatus] = useState("");
  const [completionDateInput, setCompletionDateInput] = useState("");
  const [fallThroughReason, setFallThroughReason] = useState("");

  // Next action derived from case status
  const nextAction = (status: string): string | null => {
    const map: Record<string, string> = {
      "":                  "Review this referral and accept or decline.",
      "accepted":          "Send client care documents and onboarding pack.",
      "client_care_sent":  "Await signed client care letter and ID documents.",
      "id_requested":      "Chase client for ID if not yet received.",
      "id_received":       "Open file and progress matter. Order searches if applicable.",
      "searches_ordered":  "Await search results.",
      "searches_received": "Review results and raise enquiries with seller's solicitor.",
      "enquiries_raised":  "Await replies to enquiries.",
      "enquiries_replied": "Review replies and report on title to client.",
      "report_on_title":   "Prepare for exchange. Obtain signed contract and deposit.",
      "exchange_ready":    "Confirm exchange arrangements with all parties.",
      "exchanged":         "Prepare completion statement and confirm completion date.",
      "completion_ready":  "Arrange completion funds and confirm with all parties.",
      "completed":         "Generate invoice and archive matter.",
      "on_hold":           "Await update from client or third party before progressing.",
      "withdrawn":         "No further action. Matter withdrawn.",
      "fallen_through":    "No further action. Matter has fallen through.",
    };
    return map[status] ?? null;
  };

  const action = nextAction(caseStatus);

  // When a status is selected from the dropdown, either confirm immediately or
  // capture required extra data first
  const handleStatusSelect = (status: string) => {
    if (!status) return;
    if (status === "exchanged" || status === "fallen_through") {
      // These need extra data — show inline form
      setPendingStatus(status);
      setCompletionDateInput("");
      setFallThroughReason("");
    } else {
      // All other statuses — apply immediately
      onUpdateCaseStatus(status);
    }
  };

  const confirmPendingStatus = () => {
    if (pendingStatus === "exchanged") {
      onUpdateCaseStatus(pendingStatus, undefined, completionDateInput || undefined);
    } else if (pendingStatus === "fallen_through") {
      if (!fallThroughReason) return; // reason required
      onUpdateCaseStatus(pendingStatus, fallThroughReason, undefined);
    }
    setPendingStatus("");
    setCompletionDateInput("");
    setFallThroughReason("");
  ...
  const handleSave = async () => {
    ...
    setSaveMsg("");
    try {
      const res = await fetch("/api/firm-amend-referred-quote", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${firmToken}`,
        ...
        body: JSON.stringify({
          reference: ref,
          legalFees:     editLegal.filter((f) => f.label.trim() && Number(f.amount) > 0),
          disbursements: editDisb.filter((d) => d.label.trim() && Number(d.amount) > 0),
          sdltAmount: editSdlt ? Number(editSdlt) : undefined,
        }),
      });
      const result = await res.json() as { success: boolean; error?: string };
      if (result.success) {
        setSaveMsg("Quote updated successfully.");
        setEditMode(false);
        onPortalRefresh();
      } else {
  ...
  const removeItem = (list: QuoteLineItem[], setList: (v: QuoteLineItem[]) => void, index: number) =>
    setList(list.filter((_, i) => i !== index));

  const addItem = (list: QuoteLineItem[], setList: (v: QuoteLineItem[]) => void) =>
    ...
    <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: "12px", overflow: "hidden" }}>
      ...
      <div style={{ padding: "16px 20px", borderBottom: "1px solid #f0f2f5" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px", flexWrap: "wrap" }}>
          <div>
            {/* Property address — most important field */}
            {enq.property_address && (
              <div style={{ fontWeight: 700, fontSize: "16px", color: "var(--navy)", marginBottom: "4px" }}>
                {String(enq.property_address)}
              </div>
            )}
            <div style={{ fontWeight: enq.property_address ? 400 : 700, fontSize: enq.property_address ? "13px" : "15px", color: enq.property_address ? "#6b7280" : "var(--navy)" }}>
              {ref}
            </div>
            <div style={{ fontSize: "14px", color: "#374151", marginTop: "4px" }}>
              {String(enq.transaction_type || "").replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase())}
              {enq.tenure ? ` · ${String(enq.tenure).charAt(0).toUpperCase() + String(enq.tenure).slice(1)}` : ""}
              {enq.price ? ` · £${Number(enq.price).toLocaleString("en-GB")}` : ""}
            </div>
            {enq.client_name && (
              <div style={{ fontSize: "13px", color: "#374151", marginTop: "3px", fontWeight: 600 }}>
                {String(enq.client_name)}
              </div>
            )}
            {enq.mortgage && (
              <div style={{ fontSize: "13px", color: "#6b7280", marginTop: "2px" }}>
                {enq.mortgage === "mortgage" ? "🏦 With mortgage" : "💷 Cash purchase"}
                {enq.lender ? ` — ${String(enq.lender)}` : ""}
              </div>
            )}
            {enq.negotiator_name && (
              <div style={{ fontSize: "12px", color: "#6b7280", marginTop: "2px" }}>
                Referred by: {String(enq.negotiator_name)}
              </div>
            )}
            {Number(enq.referral_fee_payable) === 1 && (
            ...
            <div style={{ marginTop: "8px" }}>
              <button type="button"
                style={{ fontSize: "12px", padding: "4px 10px", borderRadius: "6px", border: "1px solid var(--navy)", background: detailOpen ? "var(--navy)" : "transparent", color: detailOpen ? "#fff" : "var(--navy)", cursor: "pointer", fontWeight: 600 }}
                onClick={() => setDetailOpen((o) => !o)}>
                {detailOpen ? "▾ Close Matter" : "▸ View Matter"}
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px", minWidth: "160px", alignItems: "flex-end" }}>
            {!isAccepted && !isDeclined && (
              <>
                <button type="button" className="primary-button" style={{ fontSize: "13px", width: "100%" }}
                  onClick={onOpenAcceptance}>
                  Review &amp; Accept
                </button>
                <button type="button" className="muted-button" style={{ fontSize: "13px", width: "100%" }}
                  onClick={() => onDecline("")}>Decline</button>
              </>
            )}
            {isDeclined && (
              <span style={{ display: "inline-block", padding: "4px 10px", borderRadius: "12px", fontSize: "13px", fontWeight: 600, background: "#fee2e2", color: "#991b1b" }}>
                ✗ Declined
              </span>
            )}
            {isAccepted && !isCompleted && (
              <>
                <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: "10px", fontSize: "12px", fontWeight: 600, background: "#d1fae5", color: "#065f46" }}>
                  ✓ Accepted
                </span>
                {pendingStatus === "exchanged" && (
                  <div style={{ width: "100%", background: "#f0f7ff", border: "1px solid #c7d7f5", borderRadius: "8px", padding: "10px 12px" }}>
                    <div style={{ fontSize: "12px", fontWeight: 600, color: "#1e40af", marginBottom: "6px" }}>Target completion date</div>
                    <input type="date" value={completionDateInput}
                    ...
                    <div style={{ display: "flex", gap: "6px" }}>
                      <button type="button" className="primary-button" style={{ fontSize: "12px", flex: 1 }}
                        onClick={confirmPendingStatus}>Confirm Exchange</button>
                      <button type="button" className="muted-button" style={{ fontSize: "12px" }}
                        onClick={() => setPendingStatus("")}>Cancel</button>
                    </div>
                  </div>
                )}
                {pendingStatus === "fallen_through" && (
                  <div style={{ width: "100%", background: "#fff1f2", border: "1px solid #fecdd3", borderRadius: "8px", padding: "10px 12px" }}>
                    ...
                    <select value={fallThroughReason} onChange={(e) => setFallThroughReason(e.target.value)}
                      ...
                      <option value="mortgage refused">Mortgage refused</option>
                      <option value="chain collapse">Chain collapse</option>
                      <option value="other">Other</option>
                    </select>
                    <div style={{ display: "flex", gap: "6px" }}>
                      <button type="button" className="primary-button" style={{ fontSize: "12px", flex: 1 }}
                        disabled={!fallThroughReason}
                        onClick={confirmPendingStatus}>Confirm</button>
                      <button type="button" className="muted-button" style={{ fontSize: "12px" }}
                        onClick={() => setPendingStatus("")}>Cancel</button>
                    </div>
                  </div>
        ...
        {/* Case status pill + ETA */}
        <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap", marginTop: "10px" }}>
          {caseStatus && (
            <span style={{ fontSize: "12px", background: "#f0f4ff", padding: "3px 10px", borderRadius: "8px", color: "#1e40af", fontWeight: 600 }}>
              {caseStatus.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase())}
            </span>
          )}
          {!isCompleted && (
            <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
              {etaDate && !showEtaPicker && (
                <span style={{ fontSize: "12px", color: "#5b21b6", fontWeight: 600 }}>
                  📅 Est. completion: {etaDate}
                </span>
              )}
              {etaSaved && !showEtaPicker && (
                <span style={{ fontSize: "12px", color: "#065f46", fontWeight: 600, background: "#d1fae5", border: "1px solid #6ee7b7", borderRadius: "10px", padding: "2px 10px", display: "inline-flex", alignItems: "center", gap: "4px" }}>
                  ✓ Saved
                </span>
              )}
              {showEtaPicker ? (
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <input type="date" value={etaDateInput}
                    onChange={(e) => setEtaDateInput(e.target.value)}
                    style={{ fontSize: "12px", padding: "3px 6px", border: "1px solid #d1d5db", borderRadius: "5px" }} />
                  <button type="button"
                    style={{ fontSize: "11px", padding: "3px 8px", borderRadius: "4px", border: "none", background: etaSaving ? "#6b7280" : "var(--navy)", color: "#fff", cursor: etaSaving || !etaDateInput ? "not-allowed" : "pointer", opacity: !etaDateInput ? 0.5 : 1 }}
                    disabled={!etaDateInput || etaSaving}
                    onClick={async () => {
                      setEtaSaving(true);
                      setEtaSaved(false);
                      await onSetEta(etaDateInput);
                      setEtaSaving(false);
                      setEtaSaved(true);
                      setShowEtaPicker(false);
                      setEtaDateInput("");
                      setTimeout(() => setEtaSaved(false), 4000);
                    }}>
                    {etaSaving ? "Saving…" : "Save ETA"}
                  </button>
                  <button type="button"
                    style={{ fontSize: "11px", padding: "3px 6px", borderRadius: "4px", border: "1px solid #d1d5db", background: "#f9fafb", cursor: "pointer", color: "#374151" }}
                    onClick={() => { setShowEtaPicker(false); setEtaDateInput(""); }}>
                    Cancel
                  </button>
                </div>
              ) : (
                <button type="button"
                  style={{ fontSize: "11px", padding: "2px 8px", borderRadius: "4px", border: "1px solid #d1d5db", background: "#f9fafb", cursor: "pointer", color: "#374151" }}
                  onClick={() => { setShowEtaPicker(true); setEtaSaved(false); }}>
                  {etaDate ? "Change date" : "Set completion date"}
                </button>
              )}
            </div>
          )}
          {enq.invoice_ref && (
            <span style={{ fontSize: "12px", color: "#6b7280" }}>Invoice: {String(enq.invoice_ref)}</span>
          )}
        </div>
      </div>

      {/* ── View Matter detail panel ──────────────────────────────────────── */}
      {detailOpen && (
        <div style={{ background: "#f8fafc", borderTop: "1px solid #e5e7eb", borderBottom: "1px solid #e5e7eb", padding: "0" }}>

          {/* ── Section 1: Matter ── */}
          <div style={{ padding: "20px 20px 16px", borderBottom: "1px solid #e5e7eb" }}>
            <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: "#6b7280", marginBottom: "10px" }}>Matter</div>
            {enq.property_address && (
              <div style={{ fontSize: "20px", fontWeight: 700, color: "var(--navy)", marginBottom: "4px", lineHeight: 1.25 }}>
                {String(enq.property_address)}
              </div>
            )}
            <div style={{ fontSize: "13px", color: "#6b7280", marginBottom: "12px" }}>
              Ref: <span style={{ fontFamily: "monospace", color: "#374151" }}>{ref}</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "10px 24px" }}>
              {[
                { label: "Transaction type", value: String(enq.transaction_type || "—").replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()) },
                { label: "Tenure", value: enq.tenure ? String(enq.tenure).charAt(0).toUpperCase() + String(enq.tenure).slice(1) : "—" },
                { label: "Price / value", value: enq.price ? `£${Number(enq.price).toLocaleString("en-GB")}` : "—" },
                ...(enq.mortgage ? [{ label: "Mortgage", value: enq.mortgage === "mortgage" ? "Yes" : "No (cash)" }] : []),
                ...(enq.lender ? [{ label: "Lender", value: String(enq.lender) }] : []),
                ...(enq.postcode ? [{ label: "Postcode", value: String(enq.postcode) }] : []),
              ].map(({ label, value }) => (
                <div key={label}>
                  <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.3px", marginBottom: "3px" }}>{label}</div>
                  <div style={{ fontSize: "13px", color: "#111827", fontWeight: label === "Price / value" ? 600 : 400 }}>{value}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ padding: "16px 20px", borderBottom: "1px solid #e5e7eb" }}>
            <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: "#6b7280", marginBottom: "10px" }}>Client</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "10px 24px" }}>
              {[
                { label: "Name", value: String(enq.client_name || "—") },
                { label: "Email", value: String(enq.client_email || "—") },
                { label: "Phone", value: String(enq.client_phone || "—") },
              ].map(({ label, value }) => (
                <div key={label}>
                  <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.3px", marginBottom: "3px" }}>{label}</div>
                  <div style={{ fontSize: "13px", color: "#111827", fontWeight: label === "Name" ? 600 : 400, wordBreak: "break-word" }}>{value}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* ── Section 3: Status ── */}
          <div style={{ padding: "16px 20px", borderBottom: "1px solid #e5e7eb" }}>
            <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: "#6b7280", marginBottom: "10px" }}>Status</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "10px 24px", marginBottom: "14px" }}>
              <div>
                <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.3px", marginBottom: "5px" }}>Case status</div>
                <span style={{
                  display: "inline-block", padding: "3px 10px", borderRadius: "10px", fontSize: "12px", fontWeight: 600,
                  background: isCompleted ? "#d1fae5" : caseStatus === "fallen_through" || caseStatus === "withdrawn" ? "#fee2e2" : caseStatus === "exchanged" ? "#ede9fe" : "#dbeafe",
                  color: isCompleted ? "#065f46" : caseStatus === "fallen_through" || caseStatus === "withdrawn" ? "#991b1b" : caseStatus === "exchanged" ? "#5b21b6" : "#1e40af",
                }}>
                  {caseStatus ? caseStatus.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()) : "New"}
                </span>
              </div>
              <div>
                <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.3px", marginBottom: "3px" }}>ETA / completion</div>
                <div style={{ fontSize: "13px", color: etaDate ? "#5b21b6" : "#9ca3af", fontWeight: etaDate ? 600 : 400 }}>{etaDate || "Not set"}</div>
              </div>
              <div>
                <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.3px", marginBottom: "3px" }}>Referred</div>
                <div style={{ fontSize: "13px", color: "#111827" }}>{enq.referred_at ? new Date(String(enq.referred_at)).toLocaleDateString("en-GB") : "—"}</div>
              </div>
              {Number(enq.referral_fee_payable) === 1 && (
                <div>
                  <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.3px", marginBottom: "3px" }}>Referral fee</div>
                  <div style={{ fontSize: "13px", color: "#7c3aed", fontWeight: 600 }}>£{Number(enq.referral_fee_amount || 0).toFixed(2)}</div>
                </div>
              )}
            </div>

            {/* Next Action — prominent callout */}
            {action && (
              <div style={{
                padding: "12px 14px",
                background: isCompleted || caseStatus === "fallen_through" || caseStatus === "withdrawn" ? "#f3f4f6" : "#fffbeb",
                borderRadius: "8px",
                borderLeft: `4px solid ${isCompleted || caseStatus === "fallen_through" || caseStatus === "withdrawn" ? "#d1d5db" : "#f59e0b"}`,
              }}>
                <div style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: isCompleted || caseStatus === "fallen_through" || caseStatus === "withdrawn" ? "#9ca3af" : "#b45309", marginBottom: "4px" }}>
                  ▶ Next action
                </div>
                <div style={{ fontSize: "13px", fontWeight: 600, color: isCompleted || caseStatus === "fallen_through" || caseStatus === "withdrawn" ? "#6b7280" : "#78350f", lineHeight: 1.4 }}>
                  {action}
                </div>
              </div>
            )}
          ...
          <div style={{ padding: "12px 20px", borderBottom: "1px solid #e5e7eb", background: "#fff" }}>
            <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: "#6b7280", marginBottom: "8px" }}>Quick Actions</div>
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              {/* Edit ETA — opens the date picker in the card footer */}
              {!isCompleted && (
                <button type="button"
                  style={{ fontSize: "12px", padding: "5px 12px", borderRadius: "6px", border: "1px solid #d1d5db", background: "#f9fafb", cursor: "pointer", color: "#374151", fontWeight: 500 }}
                  onClick={() => { setDetailOpen(false); setShowEtaPicker(true); setEtaSaved(false); }}>
                  📅 Edit ETA
                </button>
              )}
              {/* View / Edit Quote — opens quote section below card */}
              <button type="button"
                style={{ fontSize: "12px", padding: "5px 12px", borderRadius: "6px", border: "1px solid #d1d5db", background: "#f9fafb", cursor: "pointer", color: "#374151", fontWeight: 500 }}
                onClick={() => { setDetailOpen(false); setQuoteOpen(true); }}>
                💰 View Quote
              </button>
              {/* Update Status — closes detail so the status dropdown in the card header is visible */}
              {isAccepted && !isCompleted && (
                <button type="button"
                  style={{ fontSize: "12px", padding: "5px 12px", borderRadius: "6px", border: "1px solid #d1d5db", background: "#f9fafb", cursor: "pointer", color: "#374151", fontWeight: 500 }}
          ...
          {hasQuote && storedQuote && (
            <div style={{ padding: "16px 20px" }}>
              <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", color: "#6b7280", marginBottom: "10px" }}>Quote Summary</div>
              <div style={{ display: "flex", gap: "24px", flexWrap: "wrap", alignItems: "flex-end" }}>
                {storedQuote.legalTotalInclVat != null && (
                  <div>
                    <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", marginBottom: "3px" }}>Legal fees (inc. VAT)</div>
                    <div style={{ fontSize: "14px", fontWeight: 700, color: "var(--navy)" }}>{fmt(storedQuote.legalTotalInclVat)}</div>
                  </div>
                )}
                {storedQuote.disbursementTotal != null && (
                  <div>
                    <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", marginBottom: "3px" }}>Disbursements</div>
                    <div style={{ fontSize: "14px", fontWeight: 700, color: "var(--navy)" }}>{fmt(storedQuote.disbursementTotal)}</div>
                  </div>
                )}
                {typeof storedQuote.sdltAmount === "number" && (
                  <div>
                    <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", marginBottom: "3px" }}>Est. SDLT</div>
                    <div style={{ fontSize: "14px", fontWeight: 700, color: "var(--navy)" }}>{fmt(storedQuote.sdltAmount)}</div>
                  </div>
                )}
                <div style={{ borderLeft: "2px solid var(--navy)", paddingLeft: "16px" }}>
                  <div style={{ fontSize: "11px", color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", marginBottom: "3px" }}>Total estimate</div>
                  <div style={{ fontSize: "18px", fontWeight: 700, color: "var(--navy)" }}>
                    {fmt(storedQuote.totalIncludingSdlt ?? storedQuote.grandTotal ?? 0)}
                  </div>
                </div>
              </div>
            </div>
      ...
      <div style={{ padding: "0 20px" }}>
        <button
          type="button"
          style={{ background: "none", border: "none", padding: "12px 0", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "var(--teal)", display: "flex", alignItems: "center", gap: "6px", width: "100%" }}
          onClick={() => { setQuoteOpen((o) => !o); if (editMode) setEditMode(false); }}
        >
          <span>{quoteOpen ? "▾" : "▸"}</span>
          {hasQuote ? "View / Edit Quote" : "Quote (not yet generated)"}
          {hasQuote && storedQuote?.grandTotal != null && (
            <span style={{ marginLeft: "auto", fontWeight: 700, color: "var(--navy)", fontSize: "14px" }}>
              {fmt(storedQuote.totalIncludingSdlt ?? storedQuote.grandTotal ?? 0)}
            </span>
          )}
        </button>
        ...
        {quoteOpen && (
          <div style={{ paddingBottom: "16px" }}>
            {!hasQuote && (
              <p className="form-note" style={{ marginTop: 0 }}>
                No quote has been generated for this matter yet. This will be created when the referrer submits the enquiry.
              </p>
            )}
            ...
            {hasQuote && !editMode && (
              <>
                ...
                <div style={{ background: "#f8fafc", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "14px 16px", marginBottom: "10px" }}>
                  <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--navy)", marginBottom: "10px" }}>Legal Fees</div>
                  {(storedQuote?.legalFees ?? []).map((f, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", padding: "4px 0", borderBottom: "1px solid #f0f2f5" }}>
                      <span style={{ color: "#374151" }}>{f.label}</span>
                      <span style={{ fontWeight: 600 }}>{fmt(f.amount)}</span>
                    </div>
                  ))}
                  {storedQuote?.vat != null && (
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", padding: "4px 0", borderBottom: "1px solid #f0f2f5" }}>
                      <span style={{ color: "#6b7280" }}>VAT (20%)</span>
                      <span style={{ fontWeight: 600 }}>{fmt(storedQuote.vat)}</span>
                    </div>
                  )}
                  {storedQuote?.legalTotalInclVat != null && (
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", padding: "6px 0", fontWeight: 700, color: "var(--navy)" }}>
                      <span>Total legal fees (inc. VAT)</span>
                      <span>{fmt(storedQuote.legalTotalInclVat)}</span>
                    </div>
                  )}

                  <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--navy)", margin: "12px 0 8px" }}>Disbursements</div>
                  {(storedQuote?.disbursements ?? []).map((d, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", padding: "4px 0", borderBottom: "1px solid #f0f2f5" }}>
                  ...
                  {storedQuote?.disbursementTotal != null && (
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", padding: "6px 0", fontWeight: 700, color: "var(--navy)" }}>
                      ...
                      <span>{fmt(storedQuote.disbursementTotal)}</span>
                    </div>
                  )}

                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px", padding: "10px 0 4px", fontWeight: 700, color: "var(--navy)", borderTop: "2px solid var(--navy)", marginTop: "8px" }}>
                    <span>Total (exc. SDLT)</span>
                    <span>{fmt(storedQuote?.grandTotal ?? 0)}</span>
                  </div>
                  {typeof storedQuote?.sdltAmount === "number" && (
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", padding: "4px 0", color: "#374151" }}>
                      <span>Estimated SDLT</span>
                      <span style={{ fontWeight: 600 }}>{fmt(storedQuote.sdltAmount)}</span>
                    </div>
                  )}
                  {storedQuote?.sdltNote && typeof storedQuote?.sdltAmount !== "number" && (
                    <div style={{ fontSize: "12px", color: "#9ca3af", marginTop: "4px" }}>SDLT: {storedQuote.sdltNote}</div>
                  )}
                  {storedQuote?.totalIncludingSdlt != null && (
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px", padding: "6px 0", fontWeight: 700, color: "var(--navy)" }}>
                      <span>Total (inc. SDLT)</span>
                      <span>{fmt(storedQuote.totalIncludingSdlt)}</span>
                    </div>
                  )}
                </div>

                <button type="button" className="muted-button" style={{ fontSize: "13px" }} onClick={openEditor}>
                  ✏️ Amend Quote
                </button>
                <p style={{ fontSize: "12px", color: "#9ca3af", margin: "6px 0 0" }}>
                  Referrers cannot amend quotes. As the assigned firm you may adjust line items for this matter.
                </p>
              </>
            )}
            ...
            {hasQuote && editMode && (
              <div>
                <p style={{ fontSize: "13px", color: "#374151", margin: "0 0 12px" }}>
                  Edit the fee items below. All totals are recalculated automatically when saved. VAT (20%) is applied to legal fees only.
                </p>

                {/* Legal fees editor */}
                <div style={{ marginBottom: "16px" }}>
                  <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--navy)", marginBottom: "8px" }}>Legal Fees</div>
                  {editLegal.map((item, i) => (
                    <div key={i} style={rowStyle}>
                      <input
                        style={{ ...inputStyle, flex: 2 }}
                        value={item.label}
                        placeholder="Description"
                        onChange={(e) => updateItem(editLegal, setEditLegal, i, "label", e.target.value)}
                      />
                      <input
                        ...
                        value={item.amount}
                        placeholder="0.00"
                        onChange={(e) => updateItem(editLegal, setEditLegal, i, "amount", e.target.value)}
                      />
                      <button type="button" style={{ fontSize: "13px", color: "#dc2626", background: "none", border: "none", cursor: "pointer", padding: "0 4px" }}
                        onClick={() => removeItem(editLegal, setEditLegal, i)}>✕</button>
                    </div>
                  ))}
                  <button type="button" style={{ fontSize: "12px", color: "var(--teal)", background: "none", border: "none", cursor: "pointer", marginTop: "6px", padding: 0 }}
                    onClick={() => addItem(editLegal, setEditLegal)}>+ Add legal fee item</button>
                </div>

                {/* Disbursements editor */}
                <div style={{ marginBottom: "16px" }}>
                  <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--navy)", marginBottom: "8px" }}>Disbursements</div>
                  {editDisb.map((item, i) => (
                    <div key={i} style={rowStyle}>
                      <input
                        style={{ ...inputStyle, flex: 2 }}
                        value={item.label}
                        placeholder="Description"
                        onChange={(e) => updateItem(editDisb, setEditDisb, i, "label", e.target.value)}
                      />
                      <input
                        style={{ ...inputStyle, width: "90px" }}
                        type="number"
                        ...
                        value={item.amount}
                        placeholder="0.00"
                        onChange={(e) => updateItem(editDisb, setEditDisb, i, "amount", e.target.value)}
                      />
                      <button type="button" style={{ fontSize: "13px", color: "#dc2626", background: "none", border: "none", cursor: "pointer", padding: "0 4px" }}
                        onClick={() => removeItem(editDisb, setEditDisb, i)}>✕</button>
                    </div>
                  ))}
                  <button type="button" style={{ fontSize: "12px", color: "var(--teal)", background: "none", border: "none", cursor: "pointer", marginTop: "6px", padding: 0 }}
                    onClick={() => addItem(editDisb, setEditDisb)}>+ Add disbursement item</button>
                </div>

                {/* SDLT override */}
                <div style={{ marginBottom: "16px" }}>
                  <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--navy)", marginBottom: "8px" }}>Estimated SDLT (optional)</div>
                  <input
                    ...
                    min="0"
                    value={editSdlt}
                    placeholder="Leave blank if subject to review"
                    onChange={(e) => setEditSdlt(e.target.value)}
                  />
                  <div style={{ fontSize: "12px", color: "#9ca3af", marginTop: "4px" }}>SDLT is paid direct to HMRC — it is shown separately and not part of your firm's fees.</div>
                </div>

                {saveMsg && (
                  <p style={{ fontSize: "13px", color: saveMsg.includes("success") ? "#065f46" : "#dc2626", margin: "0 0 10px" }}>
                    {saveMsg}
                  </p>
                )}

                <div style={{ display: "flex", gap: "10px" }}>
                  <button type="button" className="primary-button" style={{ fontSize: "13px" }} disabled={saving}
                    onClick={() => void handleSave()}>
                    {saving ? "Saving…" : "Save Amended Quote"}
                  </button>
...
function ReferrerSimpleForm({
  ...
  const handlePreview = (e: React.FormEvent) => {
    ...
    if (!form.price || isNaN(Number(form.price))) { setError("Enter a valid property price / value."); return; }
    const q = buildQuoteData({ ...form });
    setPreview(q);
    setStep("preview");
  };
    ...
    return (
      <div>
        ...
        <div style={{ background: "#f8faff", border: "1px solid #c7d7f5", borderRadius: "12px", padding: "20px 24px", marginBottom: "20px" }}>
          <h3 style={{ color: "var(--navy)", margin: "0 0 4px", fontSize: "18px" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
            <tbody>
              ...
              {typeof preview.sdltAmount === "number" && (
                <tr style={{ borderBottom: "1px solid #e5e7eb" }}>
                  <td style={{ padding: "8px 0", color: "#374151" }}>Stamp Duty (SDLT)</td>
  ...
  return (
    <form onSubmit={(e) => void handlePreview(e)}>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          <div style={{ ...fieldStyle, gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Property address <span style={{ color: "#dc2626" }}>*</span></label>
            <input style={inputStyle} type="text" value={form.property_address}
          ...
          <div className="form-grid">
            ...
            {form.mortgage === "mortgage" && (
              <div style={fieldStyle}>
                ...
                <input style={inputStyle} type="text" value={form.lender}
                  onChange={(e) => set("lender", e.target.value)} placeholder="e.g. Halifax" />
              </div>
            )}
            <div style={fieldStyle}>
            ...
            <div style={fieldStyle}>
              <label style={labelStyle}>First time buyer?</label>
              <select style={inputStyle} value={form.firstTimeBuyer} onChange={(e) => set("firstTimeBuyer", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            ...
            <div style={fieldStyle}>
              <label style={labelStyle}>UK resident for SDLT?</label>
              <select style={inputStyle} value={form.ukResidentForSdlt} onChange={(e) => set("ukResidentForSdlt", e.target.value)}>
                <option value="yes">Yes</option>
                <option value="no">No (2% surcharge)</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>New build?</label>
              <select style={inputStyle} value={form.newBuild} onChange={(e) => set("newBuild", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Gifted deposit?</label>
              <select style={inputStyle} value={form.giftedDeposit} onChange={(e) => set("giftedDeposit", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Buy to let?</label>
              <select style={inputStyle} value={form.buyToLet} onChange={(e) => set("buyToLet", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Shared ownership scheme?</label>
            ...
            <div style={fieldStyle}>
              <label style={labelStyle}>Help to Buy / government scheme?</label>
              <select style={inputStyle} value={form.helpToBuy} onChange={(e) => set("helpToBuy", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Lifetime ISA?</label>
              <select style={inputStyle} value={form.lifetimeIsa} onChange={(e) => set("lifetimeIsa", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Right to Buy?</label>
              <select style={inputStyle} value={form.rightToBuy} onChange={(e) => set("rightToBuy", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Buying via company?</label>
              <select style={inputStyle} value={form.isCompany} onChange={(e) => set("isCompany", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
      ...
      {isSale && (
        <div style={sectionStyle}>
          ...
          <div className="form-grid">
            ...
            <div style={fieldStyle}>
              <label style={labelStyle}>Management company / service charge?</label>
              <select style={inputStyle} value={form.managementCompany} onChange={(e) => set("managementCompany", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Property currently tenanted?</label>
              <select style={inputStyle} value={form.tenanted} onChange={(e) => set("tenanted", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Number of sellers</label>
              <select style={inputStyle} value={form.numberOfSellers} onChange={(e) => set("numberOfSellers", e.target.value)}>
                <option value="1">1 seller</option>
                <option value="2">2 sellers</option>
        ...
        <div style={sectionStyle}>
          ...
          <div className="form-grid">
            <div style={fieldStyle}>
              <label style={labelStyle}>Ownership type</label>
              <select style={inputStyle} value={form.ownershipType} onChange={(e) => set("ownershipType", e.target.value)}>
                <option value="individual">Sole name</option>
                <option value="joint">Joint names</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Lender (new)</label>
              <input style={inputStyle} type="text" value={form.lender}
                onChange={(e) => set("lender", e.target.value)} placeholder="e.g. Nationwide" />
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Additional borrowing?</label>
              <select style={inputStyle} value={form.additionalBorrowing} onChange={(e) => set("additionalBorrowing", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div style={fieldStyle}>
              <label style={labelStyle}>Transfer of equity at same time?</label>
              <select style={inputStyle} value={form.remortgageTransfer} onChange={(e) => set("remortgageTransfer", e.target.value)}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
      ...
      {/* ── Section: Transfer of Equity details ── */}
      {isTransfer && (
        <div style={sectionStyle}>
          ...
          <div className="form-grid">
            ...
            <div style={fieldStyle}>
              <label style={labelStyle}>How many owners changing?</label>
              <select style={inputStyle} value={form.ownersChanging} onChange={(e) => set("ownersChanging", e.target.value)}>
                <option value="one">One owner</option>
                <option value="two">Two owners</option>
                <option value="more">More than two</option>
              </select>
        ...
        <div className="form-grid">
          <div style={fieldStyle}>
            ...
            <input style={inputStyle} type="text" value={form.name}
              onChange={(e) => set("name", e.target.value)} placeholder="Client full name" />
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>Email <span style={{ color: "#dc2626" }}>*</span></label>
            <input style={inputStyle} type="email" value={form.email}
              onChange={(e) => set("email", e.target.value)} placeholder="client@example.com" required />
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>Phone</label>
            <input style={inputStyle} type="tel" value={form.phone}
              onChange={(e) => set("phone", e.target.value)} placeholder="07700 900000" />
          </div>
          <div style={fieldStyle}>
      ...
      <p style={{ fontSize: "12px", color: "var(--muted)", marginTop: "12px", textAlign: "right" }}>
        You'll see the full quote breakdown before anything is sent to your client.
      </p>
    </form>
  );
}
