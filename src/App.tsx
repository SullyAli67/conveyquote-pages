...
type AdminTab = "dashboard" | "enquiries" | "firms" | "lenders" | "quote" | "settings" | "referrers" | "invoices" | "pipeline";
...
function App() {
  ...
  // Invoices state
  type InvoiceRow = { reference: string; invoice_ref: string; invoice_json: string; client_name: string; assigned_firm_name: string; case_status: string; created_at: string };
  ...
  // Pipeline state
  type PipelineRow = { reference: string; client_name: string; transaction_type: string; assigned_firm_name: string; case_status: string; eta_date: string; firm_response: string; invoice_ref: string; referrer_id: number; created_at: string };
  ...
  const [referrerPortalTab, setReferrerPortalTab] = useState<"enquiries" | "new_enquiry">("enquiries");
  ...
  const handleAdminTabChange = async (tab: AdminTab) => {
    ...
    if (tab !== "quote" && tab !== "referrers" && tab !== "invoices" && tab !== "pipeline" && tab !== "settings") {
      await loadDashboardData();
    }
  ...
  const handleUpdateCaseStatus = async (reference: string, status: string, notes = "") => {
    ...
    try {
      const res = await fetch("/api/firm-update-case-status", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${firmToken}` },
        body: JSON.stringify({ reference, status, notes }),
      });
      const result = await res.json();
  ...
  const loadAllInvoices = async () => {
    ...
    try {
      ...
      const result = await res.json();
      if (result.success) {
        setAllInvoices((result.enquiries || []).filter((e: { invoice_ref?: string }) => e.invoice_ref));
      }
    } catch {}
  ...
  const buildFirmQuoteData = () => {
    ...
    return { legalFees, disbursements, vat, legalFeesExVat, disbursementTotal, grandTotal };
  };

  const handleCreateFirmQuote = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  return (
    <div className="page">
                                ...
                                <div key={q.internal_reference} className="detail-row">
                                  ...
                                  <div className="detail-row__value">
                                    <div style={{
                                      display: "inline-block", padding: "2px 8px", borderRadius: "10px",
                                      fontSize: "12px", fontWeight: 600,
                                      background: q.status === "accepted" ? "#d1fae5" : q.status === "sent" ? "#dbeafe" : "#f3f4f6",
                                      color: q.status === "accepted" ? "#065f46" : q.status === "sent" ? "#1e40af" : "#6b7280",
                                    }}>
                                      {q.status === "accepted" ? "✓ Accepted" : q.status === "sent" ? "Sent" : "Draft"}
                                    </div>
                                    <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginTop: "8px" }}>
                                      {q.status !== "accepted" && (
                                        <button type="button" className="muted-button" style={{ fontSize: "12px" }}
                                          ...
                                          {q.status === "sent" ? "Resend" : "Send to Client"}
                                        </button>
                                      )}
                                      <button type="button" className="muted-button" style={{ fontSize: "12px" }}
                                        onClick={() => {
                        ...
                        <form onSubmit={(e) => void handleCreateFirmQuote(e)}>
                          ...
                          <div style={{ marginBottom: "18px" }}>
                            ...
                            <p className="form-note" style={{ marginBottom: "10px" }}>
                              Pre-filled from your saved fee settings for {firmQuoteForm.transaction_type}. Adjust as needed.
                            </p>
                              ...
                              return (
                                <div style={{ marginTop: "14px", padding: "14px 18px", background: "#f7f9fc", borderRadius: "10px", border: "1px solid var(--border)" }}>
                                  ...
                                  <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid var(--border)", paddingTop: "8px", marginTop: "6px", fontWeight: 700, fontSize: "15px", color: "var(--navy)" }}>
                                    ...
                                    <span>£{qd.grandTotal.toFixed(2)}</span>
                                  </div>
                                </div>
                              );
                            })()}
              ...
              <div className="admin-stack" style={{ marginTop: "20px" }}>
                <SummaryCard title="Case Pipeline — All Active Matters">
                  <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "16px", alignItems: "center" }}>
                    ...
                    {["all", "accepted", "id_requested", "id_received", "searches_ordered", "searches_received", "enquiries_raised", "enquiries_replied", "exchanged", "completed", "on_hold", "withdrawn"].map((s) => (
                          ...
                          <div key={c.reference} className="detail-row">
                            <div className="detail-row__label">
                              ...
                              <div style={{ fontSize: "12px", color: "var(--muted)" }}>
                                ...
                                {c.price ? ` · £${Number(c.price).toLocaleString("en-GB")}` : ""}
                              </div>
                              {(c as Record<string, unknown>).referrer_name && (
                                <div style={{ fontSize: "11px", color: "#7c3aed" }}>
                                  Via: {String((c as Record<string, unknown>).referrer_name)}
                                </div>
                              )}
                            </div>
                            <div className="detail-row__value">
                              ...
                              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center" }}>
                                <span style={{
                                  ...
                                  background: c.case_status === "completed" ? "#d1fae5" : c.case_status === "withdrawn" ? "#fee2e2" : "#dbeafe",
                                  color: c.case_status === "completed" ? "#065f46" : c.case_status === "withdrawn" ? "#991b1b" : "#1e40af",
                                }}>
                                ...
                                {eta && (
                                  <span style={{ fontSize: "11px", color: overdue ? "#dc2626" : "var(--muted)", fontWeight: overdue ? 700 : 400 }}>
                                    {overdue ? "⚠ Overdue" : "ETA"}: {eta.toLocaleDateString("en-GB")}
                                  </span>
                                )}
                                {c.invoice_ref && (
              ...
              <div className="admin-stack" style={{ marginTop: "20px" }}>
                {selectedInvoice ? (
                  <SummaryCard title={`Invoice ${String(selectedInvoice.invoice_ref || "")}`}>
                    <button type="button" className="muted-button" style={{ minHeight: 36, padding: "0 14px", fontSize: "13px", marginBottom: "16px" }}
                      onClick={() => setSelectedInvoice(null)}>
                      ← Back to list
                    </button>
                    <button type="button" className="primary-button" style={{ minHeight: 36, padding: "0 14px", fontSize: "13px", marginBottom: "16px", marginLeft: "8px" }}
                  ...
                  <SummaryCard title="All Invoices">
                    ...
                    {!isLoadingInvoices && allInvoices.length > 0 && (
                      <div className="detail-table">
                        {allInvoices.map((inv) => {
                          ...
                          try { invoiceDetail = JSON.parse(inv.invoice_json || "{}"); } catch {}
                          return (
                            <div key={inv.reference} className="detail-row">
                              <div className="detail-row__label">
                                <strong style={{ color: "var(--teal-dark)", cursor: "pointer" }}
                                  onClick={() => invoiceDetail && setSelectedInvoice(invoiceDetail)}>
                                  {inv.invoice_ref}
                                </strong>
                              ...
                              <div className="detail-row__value">
                                ...
                                {invoiceDetail && (
                                  <button type="button" className="muted-button" style={{ minHeight: 30, padding: "0 10px", fontSize: "12px", marginTop: "6px" }}
                                    onClick={() => setSelectedInvoice(invoiceDetail)}>
                                    View
                                  </button>
                                )}
                              </div>
                    ...
                    {allInvoices.length > 0 && (() => {
                      ...
                      allInvoices.forEach((inv) => {
                        try { totalGross += Number(JSON.parse(inv.invoice_json || "{}").total_gross || 0); } catch {}
                      });
                      return (
                        <div style={{ marginTop: "16px", padding: "12px 16px", background: "#f0fdf4", borderRadius: "8px", border: "1px solid #86efac" }}>
                          <strong style={{ color: "#065f46" }}>Total invoiced: £{totalGross.toFixed(2)}</strong>
                        </div>
                      );
                    })()}
                  </SummaryCard>
      ...
      {(isReferrerPortalPage || (isReferrerLoginPage && referrerSession)) && referrerSession && (
        <div className="public-page" style={{ maxWidth: 900 }}>
          <div className="card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
              <div>
                <h2 style={{ color: "var(--navy)", margin: 0 }}>{referrerSession.referrer_name}</h2>
                <p style={{ color: "var(--muted)", margin: "4px 0 0" }}>Referrer portal</p>
              </div>
              <button type="button" className="muted-button" style={{ minHeight: 40, padding: "0 16px" }}
                onClick={() => {
            ...
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "20px" }}>
              {(["enquiries", "new_enquiry"] as const).map((tab) => (
                <button key={tab} type="button"
                  className={referrerPortalTab === tab ? "primary-button" : "muted-button"}
                  style={{ minHeight: 40, padding: "0 18px", fontSize: "14px" }}
                  onClick={() => {
                    setReferrerPortalTab?.(tab);
                    if (tab === "enquiries") {
                      fetch("/api/referrer-portal-data", { headers: { Authorization: `Bearer ${referrerToken}` } })
                        .then((r) => r.json()).then((d) => { if (d.success) setReferrerPortalData?.(d); });
                    }
                  }}>
                  {tab === "enquiries" ? "My Referrals" : "Submit New Enquiry"}
                </button>
              ))}
            </div>

            {referrerPortalTab === "enquiries" && (
              <>
                {!referrerPortalData && <p className="form-note">Loading…</p>}
                {referrerPortalData && (
                  <>
                    {referrerPortalData.referrer.referral_fee > 0 && (
                      <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: "10px", padding: "12px 16px", marginBottom: "16px" }}>
                        <strong style={{ color: "#15803d" }}>Your referral fee: £{Number(referrerPortalData.referrer.referral_fee).toFixed(2)} per completed matter</strong>
                      </div>
                    )}
                    {referrerPortalData.enquiries.length === 0 ? (
                      <p className="form-note">No referrals submitted yet. Use the Submit New Enquiry tab to get started.</p>
                    ) : (
                      <div className="detail-table">
                        {referrerPortalData.enquiries.map((enq) => (
                          <div key={enq.reference} className="detail-row">
                            <div className="detail-row__label">
                              <strong>{enq.reference}</strong>
                              <div style={{ fontSize: "13px", color: "var(--muted)" }}>{enq.client_name || enq.client_email}</div>
                              <div style={{ fontSize: "13px" }}>{String(enq.transaction_type || "").replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}</div>
                              {enq.price && <div style={{ fontSize: "13px" }}>£{Number(enq.price).toLocaleString("en-GB")}</div>}
                            </div>
                            <div className="detail-row__value">
                              <div style={{ marginBottom: "6px" }}>
                                <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: "10px", fontSize: "12px", fontWeight: 600, background: enq.case_status === "completed" ? "#d1fae5" : enq.assigned_firm_name ? "#dbeafe" : "#f3f4f6", color: enq.case_status === "completed" ? "#065f46" : enq.assigned_firm_name ? "#1e40af" : "#6b7280" }}>
                                  {enq.case_status ? enq.case_status.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) : enq.assigned_firm_name ? "Assigned" : "Pending"}
                                </span>
                              </div>
                              {enq.assigned_firm_name && <div style={{ fontSize: "12px", color: "var(--muted)" }}>Firm: {enq.assigned_firm_name}</div>}
                              {enq.eta_date && <div style={{ fontSize: "12px", color: "var(--muted)" }}>ETA: {new Date(enq.eta_date).toLocaleDateString("en-GB")}</div>}
                              {enq.assigned_firm_name && enq.case_status !== "completed" && (
                                <button type="button" className="muted-button" style={{ minHeight: 34, padding: "0 12px", fontSize: "12px", marginTop: "8px" }}
                                  onClick={() => {
                                    ...
                                    }).then((r) => r.json()).then((res) => alert(res.success ? "Update requested. The firm has been notified." : res.error || "Failed."));
            ...
            {referrerPortalTab === "new_enquiry" && (
              <ReferrerEnquiryForm referrerToken={referrerToken ?? ""} onSuccess={() => {
                setReferrerPortalTab?.("enquiries");
                fetch("/api/referrer-portal-data", { headers: { Authorization: `Bearer ${referrerToken}` } })
                  .then((r) => r.json()).then((d) => { if (d.success) setReferrerPortalData?.(d); });
              }} />
            )}
...
// ── Referrer Enquiry Form ─────────────────────────────────────────────────────

function ReferrerEnquiryForm({ referrerToken, onSuccess }: { referrerToken: string; onSuccess: () => void }) {
  const [form, setForm] = useState({
    name: "", email: "", phone: "",
    type: "purchase",
    // Single matter
    tenure: "", price: "", postcode: "",
    // Purchase
    mortgage: "", lender: "", ownershipType: "", firstTimeBuyer: "",
    additionalProperty: "", ukResidentForSdlt: "", newBuild: "",
    sharedOwnership: "", helpToBuy: "", isCompany: "", buyToLet: "",
    giftedDeposit: "", lifetimeIsa: "",
    // Sale
    saleMortgage: "", managementCompany: "", tenanted: "", numberOfSellers: "",
    // Remortgage
    currentLender: "", newLender: "", additionalBorrowing: "", remortgageTransfer: "",
    // Transfer
    transferMortgage: "", ownersChanging: "",
    // Sale+Purchase combined
    saleTenure: "", salePrice: "", salePostcode: "",
    saleMortgageCombined: "", managementCompanyCombined: "", tenantedCombined: "", numberOfSellersCombined: "",
    purchaseTenure: "", purchasePrice: "", purchasePostcode: "",
    purchaseMortgage: "", purchaseLender: "", purchaseOwnershipType: "", purchaseFirstTimeBuyer: "",
    purchaseNewBuild: "", purchaseSharedOwnership: "", purchaseHelpToBuy: "", purchaseIsCompany: "",
    purchaseBuyToLet: "", purchaseGiftedDeposit: "", purchaseAdditionalProperty: "", purchaseUkResidentForSdlt: "",
    purchaseLifetimeIsa: "",
    // Remortgage+Transfer
    remortgageTransferTenure: "", remortgageTransferPrice: "", remortgageTransferPostcode: "",
    remortgageTransferCurrentLender: "", remortgageTransferNewLender: "", remortgageTransferAdditionalBorrowing: "",
    remortgageTransferHasMortgage: "", remortgageTransferOwnersChanging: "", remortgageTransferOwnershipType: "",
  });
  const [lenders, setLenders] = useState<{ id: number; name: string }[]>([]);
  ...
  useEffect(() => {
    fetch("/api/lenders")
      .then((r) => r.json())
      .then((d) => {
        if (d.success && Array.isArray(d.lenders)) {
          setLenders(d.lenders.map((l: Record<string, unknown>) => ({ id: Number(l.id), name: String(l.name || l.lender_name || "") })));
        }
      })
      .catch(() => {});
  }, []);
  ...
  const isSalePurchase = form.type === "sale_purchase";
  const isRemortgageTransfer = form.type === "remortgage_transfer";
  const isSingle = isPurchase || isSale || isRemortgage || isTransfer;

  const LenderSelect = ({ name, value }: { name: string; value: string }) => (
    <select name={name} value={value} onChange={handleChange}>
      <option value="">{lenders.length === 0 ? "We will confirm lender eligibility" : "Please select"}</option>
      {lenders.map((l) => <option key={l.id} value={l.name}>{l.name}</option>)}
    </select>
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setSubmitted(true);
        setMessage(`Enquiry submitted. Reference: ${result.reference}`);
  ...
  if (submitted) {
    return (
      <div style={{ textAlign: "center", padding: "32px 0" }}>
        ...
        <h3 style={{ color: "var(--navy)", margin: "0 0 8px" }}>Enquiry submitted</h3>
        <p className="form-note">{message}</p>
        <p className="form-note">Returning to your referrals list…</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={(e) => void handleSubmit(e)}>
      ...
      <div style={{ marginBottom: "20px", paddingBottom: "16px", borderBottom: "1px solid var(--border)" }}>
        <h4 style={{ margin: "0 0 12px", color: "var(--navy)" }}>Client Details</h4>
        <div className="form-grid">
          <div className="field">
      ...
      {/* ── Single matter fields ── */}
      {isSingle && (
        <div style={{ marginBottom: "20px", paddingBottom: "16px", borderBottom: "1px solid var(--border)" }}>
          <h4 style={{ margin: "0 0 12px", color: "var(--navy)" }}>Property Details</h4>
          <div className="form-grid">
            <div className="field">
              <label>Tenure</label>
            ...
            <div className="field">
              ...
              <input type="text" name="price" value={form.price} onChange={handleChange} placeholder="e.g. 325000" required />
            </div>
            <div className="field">
              <label>Property postcode</label>
              <input type="text" name="postcode" value={form.postcode} onChange={handleChange} placeholder="e.g. B15 1AA" />
            </div>
            ...
            {isPurchase && <>
              ...
              {form.mortgage === "mortgage" && (
                <div className="field">
                  <label>Lender</label>
                  <LenderSelect name="lender" value={form.lender} />
                </div>
              )}
              <div className="field">
                <label>Buyer type</label>
              ...
              <div className="field">
                <label>Additional property / buy to let?</label>
                <select name="additionalProperty" value={form.additionalProperty} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>UK resident for SDLT?</label>
                <select name="ukResidentForSdlt" value={form.ukResidentForSdlt} onChange={handleChange}>
                  <option value="yes">Yes</option>
                  <option value="no">No</option>
                </select>
              </div>
              ...
              <div className="field">
                <label>Help to Buy / government scheme?</label>
              ...
              <div className="field">
                <label>Buying via company?</label>
              ...
              <div className="field">
                <label>Gifted deposit?</label>
                <select name="giftedDeposit" value={form.giftedDeposit} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Using a Lifetime ISA?</label>
                <select name="lifetimeIsa" value={form.lifetimeIsa} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
            ...
            {isSale && <>
              ...
              <div className="field">
                <label>Number of sellers</label>
                <select name="numberOfSellers" value={form.numberOfSellers} onChange={handleChange}>
                  <option value="1">1</option>
                  <option value="2">2</option>
                  <option value="3">3 or more</option>
                </select>
              ...
              <div className="field">
                <label>Management company / service charge?</label>
                <select name="managementCompany" value={form.managementCompany} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                ...
                <select name="tenanted" value={form.tenanted} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
            ...
            {isRemortgage && <>
              <div className="field">
                ...
                <input type="text" name="currentLender" value={form.currentLender} onChange={handleChange} placeholder="e.g. Halifax" />
              </div>
              <div className="field">
                <label>New lender</label>
                <LenderSelect name="newLender" value={form.newLender} />
              </div>
              <div className="field">
                <label>Additional borrowing?</label>
                <select name="additionalBorrowing" value={form.additionalBorrowing} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Transfer of equity at same time?</label>
                <select name="remortgageTransfer" value={form.remortgageTransfer} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
            ...
            {isTransfer && <>
              <div className="field">
                ...
                <select name="transferMortgage" value={form.transferMortgage} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>How many owners are changing?</label>
                <select name="ownersChanging" value={form.ownersChanging} onChange={handleChange}>
                  <option value="one">One</option>
                  <option value="two">Two</option>
                  <option value="more">More than two</option>
                </select>
      ...
      {isSalePurchase && (
        <>
          <div style={{ marginBottom: "20px", paddingBottom: "16px", borderBottom: "1px solid var(--border)" }}>
            <h4 style={{ margin: "0 0 12px", color: "var(--navy)" }}>Sale Property</h4>
            <div className="form-grid">
              <div className="field">
                <label>Tenure</label>
                <select name="saleTenure" value={form.saleTenure} onChange={handleChange} required>
                  <option value="">Please select</option>
                  <option value="freehold">Freehold</option>
                  <option value="leasehold">Leasehold</option>
                </select>
              ...
              <div className="field">
                ...
                <input type="text" name="salePrice" value={form.salePrice} onChange={handleChange} placeholder="e.g. 250000" required />
              </div>
              <div className="field">
                <label>Sale postcode</label>
                <input type="text" name="salePostcode" value={form.salePostcode} onChange={handleChange} placeholder="e.g. B15 1AA" />
              </div>
              <div className="field">
                <label>Mortgage to redeem?</label>
                <select name="saleMortgageCombined" value={form.saleMortgageCombined} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Number of sellers</label>
                <select name="numberOfSellersCombined" value={form.numberOfSellersCombined} onChange={handleChange}>
                  <option value="1">1</option>
                  <option value="2">2</option>
                  <option value="3">3 or more</option>
                </select>
              ...
              <div className="field">
                <label>Management company?</label>
                <select name="managementCompanyCombined" value={form.managementCompanyCombined} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Tenanted?</label>
                <select name="tenantedCombined" value={form.tenantedCombined} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
          ...
          <div style={{ marginBottom: "20px", paddingBottom: "16px", borderBottom: "1px solid var(--border)" }}>
            <h4 style={{ margin: "0 0 12px", color: "var(--navy)" }}>Purchase Property</h4>
            <div className="form-grid">
              <div className="field">
                <label>Tenure</label>
                <select name="purchaseTenure" value={form.purchaseTenure} onChange={handleChange} required>
                  <option value="">Please select</option>
                  <option value="freehold">Freehold</option>
                  <option value="leasehold">Leasehold</option>
                </select>
              ...
              <div className="field">
                ...
                <input type="text" name="purchasePrice" value={form.purchasePrice} onChange={handleChange} placeholder="e.g. 325000" required />
              </div>
              <div className="field">
                <label>Purchase postcode</label>
                <input type="text" name="purchasePostcode" value={form.purchasePostcode} onChange={handleChange} placeholder="e.g. B91 1AA" />
              </div>
              <div className="field">
                <label>Mortgage or cash?</label>
                <select name="purchaseMortgage" value={form.purchaseMortgage} onChange={handleChange} required>
                  <option value="">Please select</option>
                  <option value="mortgage">Mortgage</option>
                  <option value="cash">Cash</option>
                </select>
              ...
              {form.purchaseMortgage === "mortgage" && (
                <div className="field">
                  <label>Lender</label>
                  <LenderSelect name="purchaseLender" value={form.purchaseLender} />
                </div>
              )}
              <div className="field">
                <label>Buyer type</label>
                <select name="purchaseOwnershipType" value={form.purchaseOwnershipType} onChange={handleChange}>
                  <option value="">Please select</option>
                  <option value="individual">Individual</option>
                  <option value="joint">Joint</option>
                  <option value="company">Company</option>
              ...
              <div className="field">
                <label>First time buyer?</label>
                <select name="purchaseFirstTimeBuyer" value={form.purchaseFirstTimeBuyer} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Additional property?</label>
                <select name="purchaseAdditionalProperty" value={form.purchaseAdditionalProperty} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>UK resident for SDLT?</label>
                <select name="purchaseUkResidentForSdlt" value={form.purchaseUkResidentForSdlt} onChange={handleChange}>
                  <option value="yes">Yes</option>
                  <option value="no">No</option>
                </select>
              </div>
              <div className="field">
                <label>New build?</label>
                <select name="purchaseNewBuild" value={form.purchaseNewBuild} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Shared ownership?</label>
                <select name="purchaseSharedOwnership" value={form.purchaseSharedOwnership} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Help to Buy / scheme?</label>
                <select name="purchaseHelpToBuy" value={form.purchaseHelpToBuy} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Gifted deposit?</label>
                <select name="purchaseGiftedDeposit" value={form.purchaseGiftedDeposit} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>Lifetime ISA?</label>
                <select name="purchaseLifetimeIsa" value={form.purchaseLifetimeIsa} onChange={handleChange}>
                  <option value="no">No</option>
                  <option value="yes">Yes</option>
                </select>
              </div>
      ...
      {/* ── Remortgage + Transfer combined ── */}
      {isRemortgageTransfer && (
        <div style={{ marginBottom: "20px", paddingBottom: "16px", borderBottom: "1px solid var(--border)" }}>
          <h4 style={{ margin: "0 0 12px", color: "var(--navy)" }}>Remortgage and Transfer of Equity</h4>
          <div className="form-grid">
            <div className="field">
              <label>Tenure</label>
              <select name="remortgageTransferTenure" value={form.remortgageTransferTenure} onChange={handleChange} required>
                <option value="">Please select</option>
                <option value="freehold">Freehold</option>
                <option value="leasehold">Leasehold</option>
              </select>
            ...
            <div className="field">
              <label>Property value (£)</label>
              <input type="text" name="remortgageTransferPrice" value={form.remortgageTransferPrice} onChange={handleChange} placeholder="e.g. 300000" required />
            </div>
            <div className="field">
              <label>Postcode</label>
              <input type="text" name="remortgageTransferPostcode" value={form.remortgageTransferPostcode} onChange={handleChange} placeholder="e.g. B15 1AA" />
            </div>
            <div className="field">
              <label>Current lender</label>
              <input type="text" name="remortgageTransferCurrentLender" value={form.remortgageTransferCurrentLender} onChange={handleChange} placeholder="e.g. Halifax" />
            </div>
            <div className="field">
              <label>New lender</label>
              <LenderSelect name="remortgageTransferNewLender" value={form.remortgageTransferNewLender} />
            </div>
            <div className="field">
              <label>Additional borrowing?</label>
              <select name="remortgageTransferAdditionalBorrowing" value={form.remortgageTransferAdditionalBorrowing} onChange={handleChange}>
                <option value="no">No</option>
                <option value="yes">Yes</option>
              </select>
            </div>
            <div className="field">
              <label>Is there a mortgage?</label>
              <select name="remortgageTransferHasMortgage" value={form.remortgageTransferHasMortgage} onChange={handleChange}>
                <option value="yes">Yes</option>
                <option value="no">No</option>
              </select>
            </div>
            <div className="field">
              <label>Owners changing</label>
              <select name="remortgageTransferOwnersChanging" value={form.remortgageTransferOwnersChanging} onChange={handleChange}>
                <option value="one">One</option>
                <option value="two">Two</option>
                <option value="more">More than two</option>
              </select>
            ...
            <div className="field">
              ...
              <select name="remortgageTransferOwnershipType" value={form.remortgageTransferOwnershipType} onChange={handleChange}>
                <option value="individual">Individual</option>
                <option value="joint">Joint</option>
              </select>
            </div>
      ...
      {message && !submitted && (
        <p className="form-note" style={{ marginTop: "14px", color: "#dc2626", background: "#fee2e2", padding: "10px 14px", borderRadius: "8px" }}>
          {message}
        </p>
      )}

      <div className="form-footer action-row" style={{ marginTop: "20px" }}>
        <p className="form-note">This enquiry will be submitted on behalf of your client and reviewed by the ConveyQuote team.</p>
        <button type="submit" className="primary-button" disabled={submitting}>
          {submitting ? "Submitting…" : "Submit Client Enquiry"}
        </button>
      </div>
    </form>
  );
