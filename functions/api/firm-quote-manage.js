...
export async function onRequestDelete(context) {
  try {
    ...
    if (quote.status === "accepted") {
      return jsonResponse({
        success: false,
        error: "Accepted quotes cannot be archived. This quote has already been agreed by the client.",
      }, 400);
    }

    // Soft-archive — preserves the record and its tokens
    const archivedAt = new Date().toISOString();
    await env.DB.prepare(
      `UPDATE firm_quotes SET status = 'archived', updated_at = ? WHERE id = ?`
    ).bind(archivedAt, quote.id).run();
