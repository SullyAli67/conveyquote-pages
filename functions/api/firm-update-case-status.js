...
export async function onRequestPost(context) {
  try {
    ...
    const { reference, status, notes, fall_through_reason, target_completion_date, eta_date_override } = await request.json();
    ...
    // ETA: explicit override takes priority, then set on first acceptance, then push if expired
    let etaDate = enquiry.eta_date || null;
    if (eta_date_override && /^\d{4}-\d{2}-\d{2}$/.test(String(eta_date_override))) {
      etaDate = String(eta_date_override);
    } else if (status === "accepted" && !etaDate) {
      etaDate = calcEta();
    } else if (etaDate && new Date() > new Date(etaDate)) {
      etaDate = calcEta();
    }
