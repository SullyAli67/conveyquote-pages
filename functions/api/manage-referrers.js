  ...
  try {
    ...
    const result = await env.DB.prepare(
      `SELECT id, referrer_name, contact_email, contact_phone,
              referral_fee, marketing_fee, portal_email, portal_active, notes, created_at
...
export async function onRequestPost(context) {
  try {
    ...
    const {
      ...
      referral_fee, marketing_fee, portal_email, portal_active, notes, password,
    } = body;
    ...
    if (id) {
      ...
      const updateParts = [
        ...
        "referral_fee = ?", "marketing_fee = ?", "portal_email = ?", "portal_active = ?",
      ...
      // Create
      const cols = ["referrer_name", "contact_email", "contact_phone", "referral_fee", "marketing_fee", "portal_email", "portal_active", "notes"];
      const vals = [
        String(referrer_name).trim(),
        contact_email || null, contact_phone || null,
        Number(referral_fee) || 0,
