...
export async function onRequestPost(context) {
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO firm_quote_tokens (firm_quote_id, token, token_type) VALUES (?, ?, 'reject')`
    ).bind(quote.id, rejectToken).run();

    const acceptUrl = `https://conveyquote.uk/api/firm-quote-accept/?token=${acceptToken}`;
    const rejectUrl = `https://conveyquote.uk/api/firm-quote-accept/?token=${rejectToken}&action=reject`;
  ...
  <div style="max-width:620px;margin:30px auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
    ...
    <div style="padding:28px 32px;">
      ...
      <div style="text-align:center;margin:0 0 28px;">
        <a href="${rejectUrl}" style="color:#6b7280;font-size:13px;text-decoration:underline;">
          Decline this quote
        </a>
      </div>

      <p>If you have any questions about this quote, please do not hesitate to contact us${firmEmail ? ` at <a href="mailto:${escapeHtml(firmEmail)}" style="color:#0f2747;">${escapeHtml(firmEmail)}</a>` : ""}.</p>
    ...
    await env.DB.prepare(
      `UPDATE firm_quotes SET status = 'sent', sent_at = ?, updated_at = ?
       WHERE internal_reference = ? AND firm_id = ?`
    ).bind(sentAt, sentAt, internal_reference, firmId).run();
