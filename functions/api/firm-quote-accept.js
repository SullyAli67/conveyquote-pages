...
export async function onRequestGet(context) {
  try {
    ...
    if (tokenRow.used || quote.status === "accepted") {
      ...
      if (action === "reject" || tokenRow.token_type === "reject") {
        if (quote.status === "rejected") {
          return new Response(
            page("Already Declined", "Already Declined", `
              <div class="icon">ℹ️</div>
              <h1>Already Declined</h1>
              <p>You have already declined this quote. If you have changed your mind, please contact <span class="firm">${escapeHtml(firmName)}</span> directly.</p>
        ...
        return new Response(
          page("Quote Declined", "Quote Declined", `
            ...
            <p>You have declined the quote from <span class="firm">${escapeHtml(firmName)}</span>. They have been notified.</p>
